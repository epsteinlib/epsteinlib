...
For a $d$-dimensional lattice $\Lambda=A\mathbb Z^d$, with $A\in \mathbb R^{d\times d}$ regular, $\mathrm x,\mathrm y \in \mathbb R^d$, and $\nu \in \mathbb C$, the Epstein zeta function is defined by the Dirichlet series

$$
Z_{\Lambda,\nu}\begin{vmatrix} \mathrm x \newline\mathrm y \end{vmatrix}
= \sum_{z \in \Lambda}{}^{'} \frac{e^{-2\pi i \mathrm y \cdot \mathrm z}}{\left| \mathrm x- \mathrm z\right|^\nu},\quad \mathrm{Re}(\nu)>d,
$$

which can be meromorphically continued to $\nu \in \mathbb C$. Here, the primed sum excludes the case $\mathrm z = \mathrm x.$
...
The Epstein zeta function admits singularities in the lattice $\mathrm x\in\Lambda$ and in the reciprocal lattice $\mathrm y\in\Lambda^*$.
To ensure numerical stability when evaluating the Epstein zeta function, we implement the following cutoffs:
- If $(\mathrm x-\mathrm z)^2 < 10^{-64}$ for some $\mathrm z \in \Lambda$, we numerically set $\mathrm x = \mathrm z$.
- Similarly, if $(\mathrm y-\mathrm k)^2 < 10^{-64}$, for some $\mathrm k \in\Lambda^*$, we numerically set $\mathrm y = \mathrm k$.

When evaluating $\mathrm x\in\Lambda$ or $\mathrm y \in\Lambda^*$, users should set $\mathrm x= \mathrm 0$ or $\mathrm y = \mathrm 0$ and use the quasi-periodicity of the Epstein zeta function.


In addition, this library includes the regularized Epstein zeta function, which is analytic around $\mathrm y=0$, and is defined via

$$
Z_{\Lambda,\nu}^{\mathrm{reg}}\begin{vmatrix} \mathrm x \newline\mathrm y \end{vmatrix} =
e^{2\pi i \mathrm x\cdot\mathrm y}
Z_{\Lambda,\nu}\left|\begin{aligned} \mathrm x \newline\mathrm y \end{aligned}\right|
-\frac{\hat{s}(\mathrm y)}{V_{\Lambda}},
$$

where $V_{\Lambda}=|\det A|$ is the volume of the elementary lattice cell, and

$$
\hat{s}_\nu(\mathrm y) = \frac{\pi^{\nu/2}}{\Gamma(\nu/2)}\Gamma\big((d-\nu)/2\big)  (\pi \mathrm y^2)^{(\nu - d)/2},\quad \nu \not\in (d+2\mathbb N_0)
$$

is the distributional Fourier transform of $\vert\mathrm z \vert^{-\nu}$, where $\Gamma$ denotes the gamma function and we adopt the choice

$$
\hat s_{d+2k}(\mathrm y)= \frac{\pi^{k+d/2}}{\Gamma(k+d/2)}\frac{(-1)^{k+1}}{k!} ( \pi \mathrm y^2 )^{k} \log (\pi  \mathrm y^{2}),\quad k\in \mathbb N_0.
$$
...
To ensure numerical stability when evaluating the regularized Epstein zeta function as a function of $\mathrm x$, we again implement the following cutoff:
- If $(\mathrm x-\mathrm z)^2 < 10^{-64}$ for some $\mathrm z \in \Lambda$, we numerically set $\mathrm x = \mathrm z$.
