 ...
 * @return number of failed tests.
 */
static int test_harmonic_h_3D_illcond(void) {
    printf("%s ", __func__);

    char path[MAX_PATH_LENGTH];
    int result =
        snprintf(path, sizeof(path), "%s/harmonic_h_3D_illcond_ref.csv", // NOLINT
                 BASE_PATH);
    if (result < 0 || result >= sizeof(path)) {
        return fprintf(stderr, "Error creating file path\n");
    }
...
int main(void) {
    ...
    failed += run_timed_test(test_harmonic_h_3D_illcond);
