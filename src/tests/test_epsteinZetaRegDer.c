...
/*!
 * @brief Benchmarks 2D epsteinZetaRegDer function for nu = dim + 2k by comparing to
 * high-precision values from mathematica prototype over a range of random
 * parameters.
 *
 * @return number of failed tests.
 * */
...
/*!
 * @brief Benchmarks 2D Epstein zeta reg derivatives by computing its taylor series.
 *
 * @return number of failed tests.
 */
int test_epsteinZetaRegDer_taylor(void) { // NOLINT
    ...
    for (int i = 0; i < 100; i++) {
        ...
        valRef = epsteinZetaRegDer(nu, dim, m, x, yPlus, alpha0);
        ...
        while (true) {
            ...
            valTaylor += mult_pow(dim, alpha, yDiff) / (double)mult_fac(dim, alpha) *
                         epsteinZetaRegDer(nu, dim, m, x, y0, alpha);
