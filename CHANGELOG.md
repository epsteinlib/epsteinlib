...
### Fixed
- Release `v0.6.0` introduced silently wrong `epsteinZetaReg` return values for $\nu\in d+ 2\mathbb N_+$ and $\boldsymbol{y}\neq\boldsymbol{0}$ on lattices with $\mathrm{det}(A) \neq 1$, which also affected `epsteinZetaAnisoReg` for $\boldsymbol{\alpha}=\boldsymbol{0}$.
- `epsteinZetaAniso` returned zero whenever some component of $\boldsymbol{\alpha}$ was odd and the same component of $\boldsymbol{x},\boldsymbol{y}$ vanished, even on lattices that are not mirror-symmetric in that component, where the value is generally nonzero.
